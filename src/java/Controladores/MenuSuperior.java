    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            if((UsuarioDTO)request.getSession().getAttribute("usuarioDTO") != null){
                ...
                request.getSession().setAttribute("permisos", permisosDepartamento);

                ArrayList<PermisoDTO> permisosEstadoDepartamento = new ArrayList<PermisoDTO>();
                permisosEstadoDepartamento = new PermisoDAO().readAll_Estado_Departamento(2, usuarioDTO.getId_departamento());
                request.getSession().setAttribute("decretos", permisosEstadoDepartamento);

                request.getSession().setAttribute("usuarioDTO", usuarioDTO);

                request.getRequestDispatcher("MenuJefeSuperior.jsp").forward(request, response);
            }else{
