 ...
 * @author note
 */
@WebServlet(name = "ValidarPermisoAdministrativo", urlPatterns = {"/ValidarPermisoAdministrativo"})
public class ValidarPermisoAdministrativo extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int diasDisponibles = usuario.getP_administrativo();
                    ...
                    if(diasDisponibles < diasSolicitados){
                        ...
                        if(diasSolicitados >= 1){
                            ...
                            if(motivo == null || motivo.equals("")){
                                ...
                                permiso.setId_usuario(usuario.getId_usuario());
                                permiso.setId_estado_permiso(1);
                                permiso.setId_tipo_permiso(1);
                                permiso.setId_motivo(permisoMotivoDAO.last().getId_motivo());
                                ...
                                permiso.setTipo(1); //TIPO ADMINISTRATIVO
                                permiso.setMotivo(permisoMotivoDAO.last().getId_motivo());*/
                                ...
                                if(permisoDAO.create(permiso)){
                                    ...
                                    if(usuarioDAO.update(usuario)){
                                        String mensajeError = "Se ingresó permiso Administrativo.";
                                        request.getSession().setAttribute("mensajeError", mensajeError);
                                        request.getRequestDispatcher("MenuFuncionario").forward(request, response);
                                    }else{
                                        String mensajeError = "Error al modificar usuario.";
                                        request.getSession().setAttribute("mensajeError", mensajeError);
                                        request.getRequestDispatcher("MenuFuncionario").forward(request, response);
                                    }
                                }else{
                                    String mensajeError = "Error al modificar permiso.";
                                    request.getSession().setAttribute("mensajeError", mensajeError);
                                    request.getRequestDispatcher("MenuFuncionario").forward(request, response);
                                }
                                //}else{
                                //    String mensajeError = "Posee un permiso pendiente entre las fechas ingresadas.";
                                //    request.getSession().setAttribute("mensajeError", mensajeError);
                                //    request.getRequestDispatcher("MenuFuncionario").forward(request, response);
                                //}
                            }
            ...
            String mensajeError = "Se produjo un error inesperado. (ValidarPermisoAdministrativo)" + ex.getMessage();
            request.getSession().setAttribute("mensajeError", mensajeError);
            request.getRequestDispatcher("MenuFuncionario").forward(request, response);
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(ValidarPermisoAdministrativo.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
